...
describe('QRCodeService', () => {
  ...
  describe('decodeBase64', () => {
    it('should decode content generated by createBase64', () => {
      const content = '000201010212520454115303840540443435802CO5906Maicol';
      const result = QRCodeService.createBase64({
        content,
        errorCorrectionLevel: 'L',
      });

      expect(QRCodeService.decodeBase64(result)).toBe(content);
    });

    it('should decode generic base64 when size is provided', () => {
      const content = '00020101021252045411';
      const result = QRCodeService.createBase64(
        {
          content,
          errorCorrectionLevel: 'L',
      ...
      expect(
        QRCodeService.decodeBase64(result, { genericImage: true, size: 77 })
      ).toBe(content);
    });
  });
});
