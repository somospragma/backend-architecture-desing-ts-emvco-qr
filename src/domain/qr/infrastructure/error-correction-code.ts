import { ErrorCorrectionLevel, L, M, Q, H } from "../value-objects/error-correction-level";

export class ErrorCorrectionCode {
  private static readonly EC_BLOCKS_TABLE = [
  ...
  private static readonly EC_CODEWORDS_TABLE = [
  ...
  static getBlocksCount(
    ...
    switch (errorCorrectionLevel) {
      case L:
        return this.EC_BLOCKS_TABLE[(version - 1) * 4 + 0];
      case M:
        return this.EC_BLOCKS_TABLE[(version - 1) * 4 + 1];
      case Q:
        return this.EC_BLOCKS_TABLE[(version - 1) * 4 + 2];
      case H:
        return this.EC_BLOCKS_TABLE[(version - 1) * 4 + 3];
      default:
        return undefined;
    }
  }

  static getTotalCodewordsCount(
    version: number,
    errorCorrectionLevel: ErrorCorrectionLevel
  ): number | undefined {
    switch (errorCorrectionLevel) {
      case L:
        return this.EC_CODEWORDS_TABLE[(version - 1) * 4 + 0];
      case M:
        return this.EC_CODEWORDS_TABLE[(version - 1) * 4 + 1];
      case Q:
        return this.EC_CODEWORDS_TABLE[(version - 1) * 4 + 2];
      case H:
        return this.EC_CODEWORDS_TABLE[(version - 1) * 4 + 3];
      default:
        return undefined;
    }
  }
